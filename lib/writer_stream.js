...
function WriterStream(nsqdHost, nsqdPort, options, getters) {
    ...
    } else if (options && options.hasOwnProperty("topic") && options.hasOwnProperty("message")) {
        self.getters.topic = fieldGetter(options.topic);
        self.getters.message = fieldGetter(options.message);
        self._write = getterWrite;
    } else {
